...
vi.mock('../src/services/streak.service.js', () => ({
    ...
    getYearlyHeatmap: vi.fn().mockResolvedValue(Array.from({ length: 365 }, (_, i) => ({
...
describe('Habit endpoints — CRUD', () => {
    ...
    it('POST /api/habits — accepts mercyDaysAllowed', async () => {
        ...
        expect(res.status).toBe(201);
        expect(prisma.habit.create).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('PUT /api/habits/:id — updates a habit', async () => {
        prisma.habit.findFirst.mockResolvedValue({ id: 'h1', userId: 'user-1' });
    ...
    it('DELETE /api/habits/:id — deletes a habit', async () => {
        prisma.habit.findFirst.mockResolvedValue({ id: 'h1', userId: 'user-1' });
        prisma.habit.delete.mockResolvedValue({ id: 'h1' });
...
describe('Habit endpoints — Log Completion', () => {
    ...
    it('POST /api/habits/:id/log — logs a habit completion', async () => {
        ...
        prisma.habitLog.findFirst.mockResolvedValue(null); // no existing log
...
//# sourceMappingURL=habit.integration.test.js.map