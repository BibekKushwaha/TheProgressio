   ...
   const validateForm = () => {
     ...
     const handleSubmit = async (e: FormEvent) => {
       ...
       try {
         ...
         const serverMessage = err?.data?.message || err?.data || err?.error || (err?.status ? `Request failed: ${err.status}` : null);
         setErrors({ email: typeof serverMessage === 'string' ? serverMessage : 'Signup failed. Please try again.' });
       }
     };
    ...
    return (
        <div className="min-h-screen grid grid-cols-1 lg:grid-cols-5">
            ...
            <div className="hidden lg:flex lg:col-span-3 bg-slate-900 relative overflow-hidden items-center justify-center p-12 order-first lg:order-last">
                <div className="absolute inset-0 bg-gradient-to-bl from-violet-900 via-slate-900 to-black opacity-80" />
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-indigo-500/30 rounded-full mix-blend-screen filter blur-[100px] animate-pulse" />

                <div className="relative z-10 max-w-2xl text-right">
                    <h1 className="text-5xl font-bold text-white mb-6 leading-tight">
                        Design your ideal<br />
                        <span className="text-transparent bg-clip-text bg-gradient-to-l from-indigo-400 to-violet-400">
                            daily routine.
                        </span>
                    </h1>
