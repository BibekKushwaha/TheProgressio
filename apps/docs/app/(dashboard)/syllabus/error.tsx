...
export default function SyllabusError({
    ...
    useEffect(() => {
        reportError(error, {
            context: 'dashboard.syllabus.error',
    ...
    return (
        <div className="flex flex-1 items-center justify-center min-h-[60vh] p-6">
            <div className="w-full max-w-md rounded-2xl border border-red-500/30 bg-red-500/10 backdrop-blur-xl p-6 text-center text-white">
                ...
                <h2 className="text-lg font-bold mt-4">Syllabus failed to load</h2>
                <p className="text-sm text-slate-300 mt-2">
                    {error.message || "An unexpected error occurred loading the syllabus graph."}
                </p>
