    ...
    const normalizedTasks = (allTasks && allTasks.length > 0)
        ? allTasks.map((task) => {
            ...
            const categoryName = task.category?.name
                || cat?.name
                || 'No category';
            const categoryColor = task.category?.colorCode || cat?.colorCode || '#6B7280';
            const normalizedStatus = task.status;
            return { ...task, categoryName, categoryColor, normalizedStatus, date: formatDueDate(task.dueDate) };
        })
        : INITIAL_TASKS.map((task) => ({
            ...task,
            categoryName: task.category,
            categoryColor: categoryOptions.find(c => c.name === task.category)?.color || '#6B7280',
            normalizedStatus: task.status === 'TODO' ? 'PENDING' : (task.status as Status),
            dueDate: null,
            date: task.date
        }));
            ...
            <PageHeader title="Planner" description="Manage your tasks and projects">
                <div className="flex gap-2">
                    <div className="relative hidden md:block w-64">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 text-center" />
                        <input
                            type="text"
                            placeholder="Search tasks..."
                            className="w-full bg-white text-gray-900 border border-white/10 rounded-lg pl-9 pr-4 py-2 text-sm focus:outline-none"
                            value={searchQuery}
